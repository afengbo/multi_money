...
class RequestScheduler(object):
    ...
    def _set_queue_cls(self, queue_type):
        ...
        :return:
        """
        from request.request_manager.utils import get_redis_queue_cls
    ...
    def add_request(self, request_obj, queue_name, filter_name=None):
        ...
        """
        # 获取请求去重对象，并进行请求去重判断
        # 如果重复:   那么记录下重复的请求，如用日志
        # 如果不重复： 根据队列名称获取请求队列，向队列中添加请求对象，添加完成后，在请求去重容器中标记该请求
